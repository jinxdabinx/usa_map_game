document.addEventListener('DOMContentLoaded', () => {
    // Placeholder for SVG paths. You must replace this with actual SVG path data.
    const statePaths = {
    ...
    'Rhode Island': 'm 883.2,170.7 -1.3,-1.1 -2.6,-1.3 -.6,-2.2 h -.8 l -.7,-2.6 -6.5,2 3.2,12.3 -.4,1.1 .4,1.8 5.6,-3.6 .1,-3 -.8,-.8 .4,-.6 -.1,-1.3 -.9,-.7 1.2,-.4 -.9,-1.6 1.8,.7 .3,1.4 .7,1.2 -1.4,-.8 1.1,1.7 -.3,1.2 -.6,-1.1 v 2.5 l .6,-.9 .4,.9 1.3,-1.5 -.2,-2.5 1.4,3.1 1,-.9 z m -4.7,12.2 h .9 l .5,-.6 -.8,-1.3 -.7,.7 z',
    ...
    const stateNamesList = document.getElementById('state-names');
    const scoreDisplay = document.getElementById('score-display');
    const messageContainer = document.getElementById('message-container');
    const fireworksContainer = document.getElementById('fireworks-container');
    
    let statesAttempted = 0;
    const totalStates = Object.keys(statePaths).length;
    let score = 0;
    let attempts = {};

    function initMap() {
        for (const state in statePaths) {
            const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            path.setAttribute('d', statePaths[state]);
            path.classList.add('state-outline');
            path.id = state.replace(/\s/g, ''); // Use a consistent ID without spaces
            path.setAttribute('data-state-name', state); // Store the full state name
            
            // Add new event listeners for highlighting

            path.addEventListener('dragenter', handleDragEnter);

            path.addEventListener('dragleave', handleDragLeave);
            // Set up drop events on the map paths
            path.addEventListener('dragover', e => e.preventDefault());
            path.addEventListener('drop', handleDrop);
            
            mapSVG.appendChild(path);

            // Initialize attempts for each state
            attempts[state] = 0;
        }
    }

    function initStatesList() {
        const states = Object.keys(statePaths).sort();
        states.forEach(state => {
            ...
            // Set up drag events on the list items
            li.addEventListener('dragstart', handleDragStart);
            
            stateNamesList.appendChild(li);
        });
    }

    function handleDragEnter(e) {

        // This is important to allow a drop on the element

        e.preventDefault();

        e.target.classList.add('highlighted-state');
    ...
    function handleDragStart(e) {
        // Set the data to be transferred. The state name is the key.
    ...
    function handleDragLeave(e) {
        ...
        e.target.classList.remove('highlighted-state');
    ...
    function handleDrop(e) {
        ...
        if (draggedStateName === targetStateName) {
            ...
            droppedTarget.classList.add('incorrect-state');
            setTimeout(() => {
                droppedTarget.classList.remove('incorrect-state');
            }, 1000); // Remove the red highlight after 1 second
    ...
    function createFirework() {
        ...
        firework.style.animation = `firework-anim 1s forwards, fade-out 0.5s 0.8s forwards`;
